...
export function SeoWizardNav({ projectId, stages }: Props) {
  const pathname = usePathname();
  const key = seoProgressSwrKey(projectId);
  const { data } = useSWR<{ progress: { stageKey: string; status: string }[] }>(
    key,
    fetcher
  );
  ...
  const statusByStage = new Map(
    (data?.progress ?? []).map((p) => [p.stageKey, p.status])
  );
  ...
  return (
    <nav className="w-full lg:w-64 shrink-0 space-y-1">
      {stages.map((stage) => {
        ...
        const status = statusByStage.get(stage.key) ?? 'pending';
        ...
        return (
          <Link key={stage.key} href={href} prefetch={false}>
            <div
